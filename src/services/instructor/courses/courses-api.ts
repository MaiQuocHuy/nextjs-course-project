...
import type { CourseDetail } from '@/types/instructor/courses/course-details';
...
export const coursesInstSlice = createApi({
  ...
  endpoints: (builder) => ({
    ...
    >({
      ...
      transformResponse: (
        response: ApiResponse<PaginatedData<CourseEnrolledStudent>>
      ) => {
        return response.data;
      },
    ...
    getCourseReviews: builder.query<
      ...
      { courseId: string; page?: number; size?: number; sort?: string }
    >({
      query: ({ courseId, page = 0, size = 10, sort = 'reviewedAt,DESC' }) => ({
        url: `/courses/${courseId}/reviews?page=${page}&size=${size}&sort=${sort}`,
        method: 'GET',
      }),
      transformResponse: (response: ApiResponse<PaginatedData<Review>>) => {
        return response.data;
      },
      providesTags: (result) =>
        result
